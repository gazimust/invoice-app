...
if submitted:
    ...
    if st.button("Generate PDF Invoice"):
        sender_folder = invoice_from.split("\n")[0].strip().replace(" ", "_")
        save_path = os.path.join(OUTPUT_DIR, sender_folder)
        os.makedirs(save_path, exist_ok=True)

        file_path = os.path.join(save_path, f"Invoice_{invoice_number}.pdf")
        pdfkit.from_string(html_out, file_path)

        st.success(f"✅ Invoice saved to {file_path}")
        st.balloons()
